...
export class RushGame{
  ...
  togglePause(now=performance.now()){
    ...
    else{const delta=Math.max(0,now-(this.pauseStarted||now));this.paused=false;if(Number.isFinite(this.deadline))this.deadline+=delta;if(this.phase==='countdown')this.countdownEnd+=delta;if(['resolving','ending'].includes(this.phase))this.autoNextAt+=delta;if(this.messageUntil)this.messageUntil+=delta;if(this.playerSoftDropReadyAt)this.playerSoftDropReadyAt+=delta;if(this.rivalSoftDropReadyAt)this.rivalSoftDropReadyAt+=delta;this.pauseStarted=0;}
