...
const COLS=10,ROWS=20,VERSION=73,MAX_HEARTS=3,CLEAR_ROW=19;
const SAVE_KEY='rush-duel-story-playtest-v73';
...
function packTargets(stage){
 ...
 const widths=specs.reduce((s,v)=>s+v.p.width,0),remaining=Math.max(0,COLS-widths),sepCount=Math.min(Math.max(0,specs.length-1),remaining);
...
<header class="story-test-hud"><button id="storyTestBack" type="button">←</button><div class="story-test-title"><small id="storyTestScene">STORY MODE V73</small><strong id="storyTestLesson">EXACT-FIT TRAINING</strong></div><div class="story-test-stats"><b id="storyTestHearts">♥♥♥</b><span id="storyTestScore">0 PTS</span></div><div class="story-test-subhud"><div class="story-test-progress"><i id="storyTestProgress"></i></div><span class="story-test-difficulty" id="storyTestDifficulty">EASY · 1 PIECE CLEAR</span></div></header>
<section class="story-test-stage" id="storyTestStage"><canvas id="storyTestCanvas" width="360" height="660"></canvas><div class="story-test-callout" id="storyTestCallout">Fit the piece into the glowing gap.</div><div class="story-test-overlay active" id="storyTestOverlay"><article class="story-test-card"><small id="storyTestOverlayEyebrow">STORY MODE V73</small><h1 id="storyTestOverlayTitle">PIXEL JOURNEY</h1><p id="storyTestOverlayText"></p><p id="storyTestOverlayStats"></p><div class="story-test-card-actions"><button id="storyTestPrimary" type="button">BEGIN</button><button id="storyTestSecondary" class="secondary" type="button">START OVER</button></div><a href="story-editor.html?v=71">OPEN STORY EDITOR</a></article></div></section>
...
const STORY_COLORS={clouds:'#386aa4',crown:'#806a23',rocket:'#6f5060',ghost:'#315c92',heart:'#7c304a',cat:'#7d522b',flame:'#78322d',saturn:'#5d3e80',turtles:'#356b60',lightning:'#716522'};
...
function buildStoryTape(){
 const out=[],band=48,gap=10;
 STAGES.forEach((s,i)=>{const base=i*(band+gap)+12,col=STORY_COLORS[s.scene]||'#395b7e';stamp(out,MASKS.cloud,4,base+3,'#355a83',.12);stamp(out,MASKS.cloud,43,base+14,'#355a83',.1);const key=s.scene==='clouds'?'cloud':s.scene==='turtles'?'turtle':s.scene;stamp(out,MASKS[key]||MASKS.cloud,24-Math.floor((MASKS[key]?.[0]?.length||9)/2),base+19,col,.22);for(let n=0;n<12;n++){const x=(n*17+i*7)%MICRO_COLS,y=base+(n*11+i*3)%band;out.push({x,y,color:col,a:.08});}});
 return out;
}
...
function buildStageBoard(){
 const s=STAGES[state.stageIndex],b=emptyBoard(),reserved=new Set();
 s.targets.forEach(t=>t.cells.forEach(([x,y])=>reserved.add(`${x},${y}`)));
 const bottomReserved=new Set();s.targets.forEach(t=>t.cells.forEach(([x,y])=>{if(y===CLEAR_ROW)bottomReserved.add(x);}));
 for(let x=0;x<COLS;x++)if(!bottomReserved.has(x))b[CLEAR_ROW][x]=cellValue(x,CLEAR_ROW,state.stageIndex);
 for(let y=16;y<=18;y++)for(let x=0;x<COLS;x++)if(!reserved.has(`${x},${y}`)&&((x+y+state.stageIndex)%3!==0))b[y][x]=cellValue(x,y,state.stageIndex+2);
 // permanent vents keep the upper shoulder rows from accidentally clearing.
 for(let y=16;y<=18;y++){for(const x of [0,9])if(!reserved.has(`${x},${y}`))b[y][x]=0;}
 return b;
}
function stage(){return STAGES[state.stageIndex];}function target(){return stage().targets[state.stepIndex];}
function setOverlay({eyebrow='STORY MODE V73',title='PIXEL JOURNEY',text='',stats='',primary='CONTINUE',secondary='START OVER',onPrimary,onSecondary}={}){$('storyTestOverlayEyebrow').textContent=eyebrow;$('storyTestOverlayTitle').textContent=title;$('storyTestOverlayText').textContent=text;$('storyTestOverlayStats').textContent=stats;$('storyTestPrimary').textContent=primary;$('storyTestSecondary').textContent=secondary;$('storyTestPrimary').onclick=()=>onPrimary&&onPrimary();$('storyTestSecondary').onclick=()=>onSecondary&&onSecondary();$('storyTestOverlay').classList.add('active');}
...
function showMenu(){const s=loadSave();state.stageIndex=s.stage;state.score=s.score;state.phase='menu';state.running=false;setOverlay({text:'The story now scrolls behind the game as small faded pixel art. Foreground pieces fit exact cavities that teach 1-piece clears, then 2, 3, 4, 5 and 6-piece clears.',stats:'10 lessons · fixed 1-second story scroll · three hearts',primary:s.stage?'CONTINUE':'BEGIN',secondary:'START OVER',onPrimary:()=>startStage(s.stage),onSecondary:()=>{state.score=0;startStage(0);}});}
...
function lockPiece(){if(!state.piece||state.phase!=='play')return;const cells=pieceCells(state.piece),t=target(),color=SHAPES[state.piece.shapeIndex].color;if(isExactTarget(cells)){
...
function drawStoryBackground(){ctx.save();ctx.beginPath();ctx.rect(BOARD_X,BOARD_Y,BOARD_W,BOARD_H);ctx.clip();ctx.fillStyle='#020816';ctx.fillRect(BOARD_X,BOARD_Y,BOARD_W,BOARD_H);for(const p of storyPixels){const ly=p.y-state.storyRow;if(ly<-2||ly>=MICRO_ROWS)continue;ctx.globalAlpha=p.a;ctx.fillStyle=p.color;ctx.fillRect(BOARD_X+p.x*MICRO,BOARD_Y+ly*MICRO,MICRO-1,MICRO-1);}ctx.globalAlpha=1;ctx.fillStyle='rgba(1,5,15,.28)';ctx.fillRect(BOARD_X,BOARD_Y,BOARD_W,BOARD_H);ctx.strokeStyle='rgba(92,137,198,.10)';ctx.lineWidth=1;for(let x=0;x<=COLS;x++){ctx.beginPath();ctx.moveTo(BOARD_X+x*CELL+.5,BOARD_Y);ctx.lineTo(BOARD_X+x*CELL+.5,BOARD_Y+BOARD_H);ctx.stroke();}for(let y=0;y<=ROWS;y++){ctx.beginPath();ctx.moveTo(BOARD_X,BOARD_Y+y*CELL+.5);ctx.lineTo(BOARD_X+BOARD_W,BOARD_Y+y*CELL+.5);ctx.stroke();}ctx.restore();}
...
function drawForeground(){if(!state.board)return;for(let y=0;y<ROWS;y++)for(let x=0;x<COLS;x++){const v=state.board[y][x];if(v)drawCell(x,y,SHAPES[v-1].color,1,.96);}if(state.phase==='clear'){const pulse=.45+.35*Math.sin(performance.now()/45);ctx.fillStyle=`rgba(255,255,255,${pulse})`;ctx.fillRect(BOARD_X,BOARD_Y+CLEAR_ROW*CELL,BOARD_W,CELL);}
 if(state.phase==='play'&&state.stepIndex<stage().targets.length){const assist=Math.max(.18,.62-state.stageIndex*.045);stage().targets.slice(state.stepIndex).forEach((t,i)=>{for(const [x,y] of t.cells){ctx.globalAlpha=i===0?assist:assist*.22;ctx.fillStyle=i===0?stage().accent:'#74a7cc';ctx.fillRect(BOARD_X+x*CELL+5,BOARD_Y+y*CELL+5,CELL-10,CELL-10);ctx.strokeStyle=i===0?'#dff8ff':'#5a7d9c';ctx.lineWidth=i===0?2:1;ctx.strokeRect(BOARD_X+x*CELL+4.5,BOARD_Y+y*CELL+4.5,CELL-9,CELL-9);}ctx.globalAlpha=1;});}
 if(state.piece){const cells=pieceCells(state.piece);for(const [x,y] of cells)if(y>=0)drawCell(x,y,SHAPES[state.piece.shapeIndex].color,1,1);}
}
...
$('storyTestBack').addEventListener('click',()=>location.href='./?v=73');
document.addEventListener('visibilitychange',()=>{if(document.hidden&&state.running&&!state.paused)togglePause();});
window.__rushStoryV73={version:VERSION,state,stages:STAGES,startStage,input,profile};
