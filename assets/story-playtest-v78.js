...
const COLS=10,ROWS=20,VERSION=78,MAX_HEARTS=3;
const SAVE_KEY='rush-duel-story-playtest-v78';
const SHAPE_INDEX={I:0,J:1,L:2,O:3,S:4,T:5,Z:6};
const DIFF=['','EASY','EASY+','INTERMEDIATE','HARD','EXPERT'];
const MEDIUM_RISE_MS=[5500,5000,4500,4000,3600,3200,2800,2400,2100,1800];
const MEDIUM_START_BASE=15,MEDIUM_DEADLINE_BASE=10,STAGE_SPACING_ROWS=4,HANDOFF_ROW_MS=150;
...
function validateCurriculum(){
 ...
 for(let i=1;i<MEDIUM_RISE_MS.length;i++)if(MEDIUM_RISE_MS[i]>=MEDIUM_RISE_MS[i-1])throw new Error('Medium reaction timer must strictly decrease.');
}
validateCurriculum();
...
function showModeMenu(){state.running=false;state.phase='menu';setOverlay({text:'Easy keeps each strategy lesson stationary. Medium turns all ten lessons into one continuous upward-moving story: the next formation is already visible underneath the current one and rises into place automatically.',stats:'Easy: learn the move · Medium: continuous conveyor with shrinking reaction time',primary:'EASY',secondary:'MEDIUM',onPrimary:()=>startCourse('easy'),onSecondary:()=>startCourse('medium')});}
...
function togglePause(){if(!state.started)return;if(state.paused){state.paused=false;state.running=true;state.last=performance.now();hideOverlay();return;}state.paused=true;state.running=false;setOverlay({eyebrow:'PAUSED',title:state.mode==='medium'?'CONTINUOUS MEDIUM':'EASY STRATEGY',text:stage().concept,stats:`LESSON ${state.stageIndex+1}/${STAGES.length}`,primary:'RESUME',secondary:'BACK',onPrimary:()=>togglePause(),onSecondary:()=>location.href='./?v=78'});}
function riseInterval(){return MEDIUM_RISE_MS[state.stageIndex]||1800;}
function recoverPieceAfterRise(){if(!state.piece)return;let guard=6;while(!canPlace(state.piece)&&guard--&&state.piece.y>-4)state.piece.y--;if(!canPlace(state.piece)){loseHeart('THE BOARD CAUGHT THE PIECE · RETRY');}}
function riseMediumBoard(){
 if(state.baseY<=MEDIUM_DEADLINE_BASE){loseHeart('REACTION WINDOW EXPIRED · ♥ LOST');if(state.lives>0){state.phase='reset';state.phaseUntil=performance.now()+650;}return;}
...
$('storyTestBack').addEventListener('click',()=>location.href='./?v=78');
document.addEventListener('visibilitychange',()=>{if(document.hidden&&state.running&&!state.paused)togglePause();});
window.__rushStoryV78={version:VERSION,state,stages:STAGES,startCourse,input,profile,validateCurriculum,landingFor,mediumRiseMs:MEDIUM_RISE_MS,stageSpacingRows:STAGE_SPACING_ROWS,upcomingBaseY};
