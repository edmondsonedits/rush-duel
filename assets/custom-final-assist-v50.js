...
const QUEUE_MARK=Symbol('customForecastQueueV50');

// PLAYER-PROTECTION RULE
// The Custom Mode NEXT panel displays exactly three upcoming pieces. V50 never
...
const SCRIPT_SRC=document.currentScript?.src||new URL('assets/custom-final-assist-v50.js',location.href).href;
...
function workerUrl(){
  return new URL('./custom-final-assist-worker-v50.js?v=50',SCRIPT_SRC).href;
}
...
Array.prototype.push=function(...items){
  ...
  // The search begins immediately after the next active piece is selected. This
  // gives the Web Worker the entire player-placement window to choose the two
  // hidden queue pieces, instead of doing expensive work during Board.lock().
...
Board.prototype.lock=function(){
  ...
  // Any plan based on the previous active piece is stale once it locks. Cancel
  // it immediately. The next queue shift/push starts a fresh plan using the real
  // board created by the player's actual placement.
